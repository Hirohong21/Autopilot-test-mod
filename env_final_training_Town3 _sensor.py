...
sys.path
print(sys.path)
sys.path.append("/home/sdc/Desktop/scenario_runner/")
import carla
print(sys.path)
import pygame
import random
...
class CarlaEnv(gym.Env):
    def __init__(self, config=ENV_CONFIG):
        ...
        self.camera_sensor_choice = None
        ...
        self.scenario_list = ['OppositeVehicleRunningRedLight', 'StationaryObjectCrossing', 'DynamicObjectCrossing',
                              'NoSignalJunctionCrossing', 'ControlLoss']
    ...
    def _restart(self):
        ...
        try:
            ...
            self.scenario_name = random.choice(self.scenario_list)#'ControlLoss'     #the name of scenario
            ...
            #
            # sensor = bp_library.find('sensor.camera.*')
            # print(sensor)
        ...
        #     sensor = random.choice(['sensor.camera.rgb','sensor.camera.semantic_segmentation','sensor.camera.depth'])
        #     camera_sensor = bp_library.find(sensor)
        #     camera_sensor.set_attribute('fov', '120')
        #     camera_sensor.set_attribute('image_size_x', str(ENV_CONFIG["x_res"]))
        #     camera_sensor.set_attribute('image_size_y', str(ENV_CONFIG["y_res"]))
        #     print('======================running '+sensor+'======================')
        #
        #
        #
        #     # here is the bug, carla will throw bad_weak_ptr() and block the process
        #     self.camera_sensor = world.spawn_actor(camera_sensor, camera_transform, attach_to=self.vehicle) # 32 !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        ...
        #     print("spawn fail, sad news", e)
            ...
            # setup sensor
            camera_transform = carla.Transform(carla.Location(x=1, y=0, z=2))
            self.sensor = random.choice(['sensor.camera.rgb', 'sensor.camera.semantic_segmentation', 'sensor.camera.depth'])
            print('======================running ' + self.sensor + '======================')
            time.sleep(1)
            camera_sensor = bp_library.find(self.sensor)
            camera_sensor.set_attribute('fov', '120')
            camera_sensor.set_attribute('image_size_x', str(ENV_CONFIG["x_res"]))
            camera_sensor.set_attribute('image_size_y', str(ENV_CONFIG["y_res"]))
            # here is the bug, carla will throw bad_weak_ptr() and block the process
            self.camera_sensor_choice = world.spawn_actor(camera_sensor, camera_transform,
                                                 attach_to=self.vehicle)  # 32 !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            self.actor_list.append(self.camera_sensor_choice)
            bp = bp_library.find('sensor.other.collision')
            self.collision_sensor = world.try_spawn_actor(bp, carla.Transform(), attach_to=self.vehicle)
            self.actor_list.append(self.collision_sensor)
            bp = bp_library.find('sensor.other.lane_detector')
   ...
   #  @set_timeout(10)
    def _reset(self):
        ...
        # set rgb,depth,segmentation camera sensor



        if self.sensor =='sensor.camera.rgb':

            self.camera_sensor_choice.listen(lambda image: self._parse_image1(weak_self, image, cc.Raw, 'rgb'))
        ...
        elif self.sensor == 'sensor.camera.depth':

            self.camera_sensor_choice.listen(lambda image: self._parse_image1(weak_self, image, cc.Raw, 'depth'))
            while len(self._image_rgb1) < 4:
        ...
        elif self.sensor == 'sensor.camera.semantic_segmentation':

            self.camera_sensor_choice.listen(lambda image: self._parse_image1(weak_self, image, cc.CityScapesPalette, 'segmentation'))
            while len(self._image_rgb1) < 4:
    ...
    def write_result(self, times):
        ...
        filename = 'Town3_training_result' + ".txt"

        if times == 1:
            with open(filename, "a+") as fd:

                fd.write('\n\n========== Start tarning TOWN3 ! '+current_time+'==========\n')
                fd.write(str(times)+'  '+self.scenario_name + ': ==========>' + str(self.total_score) + '\n')
...
if __name__ == '__main__':
    ...
    while True:
        ...
        obs, reward, done, info = env.step([1, 0])    #just go straight
        R += reward
        print(R)
